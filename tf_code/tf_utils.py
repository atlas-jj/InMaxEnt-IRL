...
# return summed value of the two tensors
# sess, current tf.session
def add_grad_theta_by_tensor( tf_grad_theta1, tf_grad_theta2, sess):
    grad_theta_sum = [tf.add(tf_grad_theta1[i], tf_grad_theta2[i]) for i in range(len(tf_grad_theta1))]
    return sess.run(grad_theta_sum)


def trans_sigmoid(x): #a translated sigmoid function, lamda, demonstrator's self-assessment is a global variable.
  #a, b = utils.low_up_bound(0)
  a=-1
  b=1
  return tf.nn.sigmoid(x) * (b-a) + a
...
def conv2d(x, n_kernel, scope="conv", k_sz=[8,8], stride=1):
  ...
  with tf.variable_scope(scope):
      W = tf.Variable(tf.random_normal([k_sz[0], k_sz[1], int(x.get_shape()[1]), n_kernel]))
